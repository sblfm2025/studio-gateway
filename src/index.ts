...
import {
  ...
  FirestoreNowPlaying,
  FirestoreGatewayHeartbeat,
  FirestoreTrackHistory,
  FirestoreAuditLog,
} from "./types";
...
async function writeAuditLog(
  ...
  try {
    ...
    Logger.error(
      `[AuditLog] Gagal menulis log audit ke Firestore: ${String(err)}`,
    );
  }
}
  ...
  try {
    ...
    if (doc.exists) {
      ...
      if (data && (data.artist || data.title)) {
        ...
        Logger.info(
          `[Sync] Inisialisasi lagu terakhir dari Firestore: "${lastTrack.artist} - ${lastTrack.title}"`,
        );
      }
    }
  } catch (err) {
    Logger.warn(
      `[Sync] Gagal inisialisasi lagu terakhir dari Firestore, akan diinisialisasi pada polling pertama: ${String(err)}`,
    );
  }
}
...
function toSafeError(error: unknown): {
  ...
  if (
    ...
    return {
      ...
      errorMessageSafe:
        "Koneksi ke API Remote Control RadioBOSS melampaui batas waktu respons (timeout).",
    };
  }
  ...
  if (message.includes("ECONNREFUSED") || message.includes("fetch failed")) {
    return {
      ...
      errorMessageSafe:
        "Gagal terhubung ke RadioBOSS. Pastikan aplikasi RadioBOSS aktif dan Remote Control API diaktifkan.",
    };
  }
  ...
  return {
    ...
    errorMessageSafe:
      "Gateway gagal melakukan sinkronisasi data dengan API RadioBOSS.",
  };
}
  ...
  try {
    ...
    if (!isTitleEmpty && !isFallbackOnly) {
      if (!lastTrack) {
        ...
        Logger.info(
          `[Sync] Registrasi trek pertama terdeteksi: "${currentArtist} - ${currentTitle}"`,
        );
      } else if (
        lastTrack.artist !== currentArtist ||
        lastTrack.title !== currentTitle
      ) {
        // Lagu telah berganti!
        ...
        try {
          const startMs =
            typeof lastTrack.startedAt.toMillis === "function"
              ? lastTrack.startedAt.toMillis()
              : lastTrack.startedAt.getTime();
          const endMs =
        ...
        if (durationMs >= 5000) {
          Logger.info(
            `[Sync] Lagu berganti dari "${lastTrack.artist} - ${lastTrack.title}" ke "${currentArtist} - ${currentTitle}"`,
          );
          ...
          Logger.warn(
            `[Sync] Pergantian lagu terlalu cepat (${durationMs / 1000} detik). Menolak penulisan sejarah untuk menghindari spam.`,
          );
        }
    ...
    Logger.error(
      `[Sync] Error saat proses sinkronisasi: ${error.message || String(error)}`,
    );
    ...
    try {
      ...
      Logger.error(
        `[Sync] Gagal menulis status error ke Firestore: ${String(dbErr)}`,
      );
    }
  }
}
...
async function updateGatewayHeartbeat(options: {
...
async function startAgent() {
  ...
  try {
    ...
    const startupLog: FirestoreAuditLog = {
      ...
      result: "success",
      details: {
        message: `Gateway agent berhasil dijalankan pada PC ${pcName}`,
      },
    };
...
async function gracefulShutdown(signal: string) {
  Logger.info(
    `[Agent] Menerima sinyal ${signal}. Memulai penutupan gateway secara normal...`,
  );
  ...
  try {
    ...
    const shutdownLog: FirestoreAuditLog = {
      ...
      result: "success",
      details: {
        message: `Gateway agent berhasil ditutup secara normal melalui sinyal ${signal}`,
      },
    };
    await addDocument("radiobossAuditLogs", shutdownLog);

    Logger.info(
      "[Agent] Detak jantung offline berhasil diperbarui. Mengakhiri proses.",
    );
  } catch (err) {
    Logger.error(
      `[Agent] Gagal memperbarui status penutupan ke Firestore: ${String(err)}`,
    );
  } finally {
    process.exit(0);
  }

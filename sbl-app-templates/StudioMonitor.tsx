...
export const StudioMonitor: React.FC<StudioMonitorProps> = ({
  db,
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            ID: {gatewayId} | PC: {heartbeat?.pcName || "Menghubungkan..."}
          </p>
        </div>
          ...
          <div style={styles.valueContainer}>
            <span
              style={{
                ...styles.dot,
                backgroundColor:
                  status?.online && isClientOnline ? "#10B981" : "#EF4444",
              }}
            />
            <span style={styles.value}>
              {status?.online && isClientOnline
                ? "Terhubung (Online)"
                : "Terputus (Offline)"}
            </span>
          </div>
      ...
      {!isClientOnline && (
        <div style={styles.errorPanel}>
          <strong style={styles.errorTitle}>
            Peringatan Koneksi Studio Terputus
          </strong>
          <p style={styles.errorText}>
            Aplikasi tidak menerima detak jantung dari gateway di PC Studio
            dalam 60 detik terakhir. Aplikasi secara otomatis beralih ke mode
            fallback siaran standar.
          </p>
        </div>
      )}
      ...
      {isClientOnline && status?.errorCode && (
        <div style={styles.errorPanel}>
          <strong style={styles.errorTitle}>
            Kesalahan API Pemutar: {status.errorCode}
          </strong>
          <p style={styles.errorText}>
            {status.errorMessageSafe ||
              "Terjadi kesalahan sinkronisasi eksternal."}
          </p>
        </div>
      )}
    </div>

 ...
 */
export function subscribeToNowPlaying(db: any, callback: (data: RadioBossNowPlaying | null) => void) {
 ...
 */
export function subscribeToPlayerStatus(db: any, callback: (data: RadioBossStatus | null) => void) {
 ...
 */
export function subscribeToGatewayHeartbeat(db: any, gatewayId: string, callback: (data: GatewayHeartbeat | null) => void) {
  ...
  return onSnapshot(docRef, (snapshot) => {
    ...
    console.error(`[Service] Gagal memantau heartbeat gateway ${gatewayId}:`, err);
    callback(null);
  });
}

...
export const StudioMonitorPage: React.FC<StudioMonitorPageProps> = ({ db }) => {
  const { artist, title, nextArtist, nextTitle, progressPercent, isLive } =
    useRadioBossNowPlaying(db, "studio-main");
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.mainSubtitle}>
        Panel Pemantauan Real-Time Siaran Radio SBL
      </p>
        ...
        <div style={styles.leftCol}>
          ...
          <div style={styles.historyCard}>
            <h3 style={styles.historyTitle}>
              10 Lagu Terakhir Selesai Diputar
            </h3>
            {history.length === 0 ? (
                ...
                {history.map((item) => (
                  <li key={item.id} style={styles.historyItem}>
                    <div style={styles.historyDetails}>
                      <strong style={styles.historyTrack}>
                        {item.artist ? `${item.artist} - ` : ""}
                        {item.title}
                      </strong>
                      <span style={styles.historyProgram}>
                        {item.programTitle || "Program tidak tersinkron"}
                      </span>
                    </div>
